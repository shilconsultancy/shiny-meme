...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_booking_status') {
    ...
    if ($booking_id === null || $booking_id === false) {
        ...
        error_log("AJAX Update (test.php): Invalid Booking ID. Received: " . ($_POST['id'] ?? 'NULL'));
    } elseif ($new_status === null || !in_array($new_status, $valid_statuses)) {
        $response['message'] = 'Invalid Status value provided.';
        error_log("AJAX Update (test.php): Invalid Status. Received: " . ($new_status ?? 'NULL') . " for Booking ID: " . $booking_id);
    } else {
        ...
        if ($conn_update->connect_error) {
            ...
            error_log("AJAX Update (test.php): DB connection error - " . $conn_update->connect_error);
        } else {
            ...
            try {
                ...
                if ($stmt_update === false) {
                    ...
                    error_log("AJAX Update (test.php): SQL prepare error - " . $conn_update->error . " (Query: " . $sql_update . ")");
                } else {
                            ...
                            if($stmt_check) {
                                ...
                                if ($result_check->num_rows > 0) {
                                    ...
                                    if ($current_booking['status'] == $new_status) {
                                        ...
                                        error_log("AJAX Update (test.php): Booking ID {$booking_id} exists but status not changed from {$current_booking['status']} to {$new_status} and affected_rows was 0.");
                                    }
                                } else {
                                    $response['message'] = "Booking #{$booking_id} not found. Status update failed.";
                                    error_log("AJAX Update (test.php): Booking ID {$booking_id} not found during update attempt.");
                                }
                                $stmt_check->close();
                            } else {
                                 $response['message'] = "Failed to verify update. Please check the booking.";
                                 error_log("AJAX Update (test.php): Failed to prepare check statement for Booking ID {$booking_id}. Error: " . $conn_update->error);
                            }
                        }
                    } else {
                        $response['message'] = "Failed to execute the status update. Please try again.";
                        error_log("AJAX Update (test.php): SQL execute error for Booking ID {$booking_id} - " . $stmt_update->error);
                    }
                    $stmt_update->close();
                }
            } catch (Exception $e) {
                $response['message'] = "A server error occurred during status update: " . $e->getMessage();
                error_log("AJAX Update (test.php): Exception for Booking ID {$booking_id} - " . $e->getMessage());
            }
            $conn_update->close();
        }
    }
...
// ini_set('error_log', '/path/to/your_project/php-page-errors.log');
...
if ($db_conn_page_load->connect_error) {
    ...
    error_log("Page Load DB Connection Error (test.php): " . $errorMessage); 
    ...
    try {
            ...
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                 ...
                 if ($total_bookings == 0 && empty($search_query)) {
                    ...
                    $message = "You've requested a page that doesn't exist. Showing the last page.";
                }
            }
            $stmt->close();
        } else {
        ...
        error_log("Page Load Data Fetch Error (test.php): " . $errorMessage); 
...
<head>
    ...
    <title>Admin Dashboard - Luxury Transfer Bookings</title>
    ...
    <style>
        ...
        .btn-primary[disabled] { background-color: #7d671f; color: #333; cursor: not-allowed; }
        ...
        select.status-select.status-Pending { color: orange !important; } select.status-select.status-Confirmed { color: #28a745 !important; } select.status-select.status-Completed { color: #17a2b8 !important; } select.status-select.status-Cancelled { color: #dc3545 !important; }
        ...
        .status-select option.status-Pending { color: orange; } .status-select option.status-Confirmed { color: #28a745; } .status-select option.status-Completed { color: #17a2b8; } .status-select option.status-Cancelled { color: #dc3545; }
        ...
        #newBookingAlertModal .modal-content { max-width: 450px; padding: 3rem 2.5rem; border: 2px solid #D4AF37; box-shadow: 0 0 30px rgba(212, 175, 55, 0.5); }
        .dropdown { position: relative; display: inline-block; }
        .dropdown-content { display: none; position: absolute; right: 0; background-color: #1a1a1a; min-width: 200px; box-shadow: 0px 8px 16px 0px rgba(0,0,0,0.6); z-index: 100; border-radius: 0.25rem; border: 1px solid rgba(212, 175, 55, 0.3); margin-top: 0.5rem; overflow: hidden; }
        .dropdown-content a { color: #ccc; padding: 12px 16px; text-decoration: none; display: flex; align-items: center; font-size: 0.875rem; transition: background-color 0.2s ease, color 0.2s ease; white-space: nowrap; }
        .dropdown-content a:hover { background-color: rgba(212, 175, 55, 0.15); color: #D4AF37; }
        .dropdown-content a i.fa-fw { margin-right: 0.75rem; width: 1.25em; text-align: center; }
...
<body class="flex flex-col">
    <header class="bg-black py-4 shadow-lg sticky top-0 z-50 border-b border-gray-800">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center">
            <h1 class="monument text-2xl font-bold gold">Admin <span class="text-white">Dashboard</span></h1>
    ...
    <main class="flex-grow max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <form action="test.php" method="GET" class="mb-8 max-w-2xl mx-auto">
            <div class="flex shadow-md rounded-md">
                ...
                <?php if (!empty($search_query)): ?>
                    <a href="test.php" class="btn btn-secondary ml-2 !px-3 md:!px-6" title="Clear Search">
        ...
        <?php elseif (!empty($message) && !empty($errorMessage)): ?>
            <div class="bg-red-700 bg-opacity-50 border border-red-700 p-4 rounded-md text-center text-white mb-6">
                ...
                <?php if (ini_get('display_errors') === '0'): ?>
                     <p class="text-xs mt-2">Details have been logged for the administrator.</p>
                <?php endif; ?>
            </div>
        <?php endif; ?>
        ...
        <?php if (!empty($bookings)): ?>
            ...
            <div class="mt-8 mb-4 flex justify-center items-center space-x-1 sm:space-x-2 text-white pagination-container">
                <?php 
                $query_params = $_GET; 
    ...
    <footer class="bg-black py-4 text-center text-gray-600 text-sm border-t border-gray-800">
        <p>© <?php echo date('Y'); ?> Luxury Transfer. All rights reserved.</p>
        <p class="text-xs mt-1">Page loaded in <?php echo round(microtime(true) - ($_SERVER["REQUEST_TIME_FLOAT"] ?? microtime(true)), 4); ?> seconds.</p>
    </footer>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const viewNewBookingBtn = document.getElementById('viewNewBookingBtn'); // Button for the "New Booking Alert Modal"
    ...
    let currentBookingIdForModal = null; // Still used by checkNewBookings to know which booking is new
    let isUpdatingStatus = false; 

    if (optionsDropdownBtn) {
    ...
    window.addEventListener('click', function(event) {
        if (optionsDropdownContent && optionsDropdownContent.classList.contains('show-dropdown')) {
            if (optionsDropdownBtn && !optionsDropdownBtn.contains(event.target) && !optionsDropdownContent.contains(event.target)) {
    ...
    function showNotification(type, title, message) {
        ...
        `; // The .notification-close div is the close button
    ...
    function openBookingDetailsModal(bookingId) {
        currentBookingIdForModal = bookingId; // Set this in case it's needed elsewhere, though not for accept/cancel now
    ...
    function updateBookingStatus(bookingId, newStatus) {
        ...
        fetch('test.php', { // Ensure this matches your PHP filename
            method: 'POST',
            headers: {
        ...
        .then(response => {
            return response.text().then(text => {
                try {
                    ...
                    throw new Error(`Unexpected server response (not JSON). Check console and server logs. Response start: ${text.substring(0,100)}`);
                }
            });
        })
        .then(data => {
    ...
    copyableFields.forEach(field => {
        field.addEventListener('click', function() {
            ...
            navigator.clipboard.writeText(textToCopy)
                .then(() => { showNotification('info', 'Copied!', `'${textToCopy.substring(0,30)}...' copied.`); })
                .catch(err => { /* Fallback copy method */ 
                    ...
                    try {
                        ...
                        showNotification('info', 'Copied!', `'${textToCopy.substring(0,30)}...' copied (fallback).`);
                    } catch (fallbackErr) {
    ...
    // MODIFIED: Event listener for the "New Booking Alert Modal" button
    if (viewNewBookingBtn && newBookingAlertModal) {
        viewNewBookingBtn.addEventListener('click', function() {
            newBookingAlertModal.classList.remove('active'); // Only close this modal
        });
    }

    // Event listeners for acceptBookingBtn and cancelBookingBtn are fully removed.
    ...
    function checkNewBookings() {
        fetch('check_new_bookings.php') 
            ...
            .then(data => {
                if (data.success && data.bookings.length > 0) {
                    ...
                    if (latestBooking.id && latestBooking.id.toString() !== lastAlertedBookingId) {
                        ...
                        // currentBookingIdForModal is set here, which is fine. 
                        // The "Close" button on the new booking alert won't use it to open details anymore.
            ...
            .catch(error => {
                // console.warn('Error checking for new bookings:', error); 
            });
    }
